...
// Mobile Menu order pass (Josh-approved order, non-Archive mobile
// refinements round): the mobile Menu overlay's own link order/labels,
// deliberately independent of MENU_LINKS above -- desktop navigation
// stays exactly as it is (4 links, its own order); only mobile gets the
// requested Archive / Projects / Practice / Contact / Search / Journal
// sequence. Split into two arrays because Search itself isn't a page
// link (see MobileMenuOverlay.jsx's own comment on why it's rendered
// separately, via onSearchOpen, not through this link table) but still
// needs to land in a specific position in the visual list --
// linksAfterSearch now holds Journal alone, so Search renders between
// Contact and Journal (MobileMenuOverlay.jsx's own
// linksBeforeSearch.map(renderLink), then the Search button, then
// linksAfterSearch.map(renderLink), unchanged).
          ...
          {isMenuOpen ? (
            ...
            <div className="index-drawer__row index-drawer__row--menu">
              {/* Navigation active-state (bracket treatment removed):
                  ...
                  color swap -- the active label just switches to the same
                  neutral gray SEARCH already uses (.text-control--muted's
